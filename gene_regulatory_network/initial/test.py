...
# Unit Test to check that GRN.develop() works
def unit_test_1():
    grn = WatsonGRN(3)
    ...
    print(f'Gene Potentials after step(): {grn.gene_potentials}')
    print(f'Interaction Matrix after step(): {grn.interaction_matrix}')

# Unit Test to check that GRN.set_random_weights() works
def unit_test_2():
    grn = WatsonGRN(3)
...
# Unit Test to check that Robot.set_inputs() works
def unit_test_3():
    robot = Robot('Walker-v0')
    robot.instantiate_controller()
    inputs = np.array([0, 1, 1, 0, 1, 0, 0, 1])

    print(f'Sensory Inputs: {inputs}')
    print(f'Robot GRN Gene Potentials: {robot.robot_controller.controller.gene_potentials}')
    print(f'Robot GRN Interaction Matrix: {robot.robot_controller.controller.interaction_matrix}')
    print()

    robot.set_inputs(inputs)

    print(f'Robot GRN Gene Potentials: after set_inputs(): {robot.robot_controller.controller.gene_potentials}')
    print(f'Robot GRN Interaction Matrix after set_inputs(): {robot.robot_controller.controller.interaction_matrix}')


# Unit Test to check that Robot.get_actuator_values() works
def unit_test_4():
    robot = Robot('Walker-v0')
    robot.instantiate_controller()

    actuator_values = robot.robot_controller.step()

    print(f'Robot GRN Gene Potentials: {robot.robot_controller.controller.gene_potentials}')
    print()

    print(f'Actuator Values: {actuator_values}')


# Unit Test to check that GeneticAlgorithm.start() works
def unit_test_5():
    parameters = {
    ...
    ga = GeneticAlgorithm(parameters['shape'], parameters['pop_size'], parameters['max_generations'],
                              parameters['environment'], exp_path, 0, False)
