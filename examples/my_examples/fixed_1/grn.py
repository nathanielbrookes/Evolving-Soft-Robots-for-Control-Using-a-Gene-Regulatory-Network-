...
class WatsonGRN:
    ...
    def set_random_weights(self):
        ...
        # Strategy 2 & 3: 10% of the matrix is initialised with random values in the range [-1, 1]
        for i in range(self.gene_count*self.gene_count):
            if random.uniform(0, 1) < 0.1:
                self.interaction_matrix[i] = random.uniform(-1, 1)
                
    
    # Strategy 3: 1% of the matrix connections are deleted; 5% are mutated by adding a value taken 
    # from the Gaussian/Normal distribution (mean=0, std=0.35)
    def mutate_weights(self):
        for i in range(self.gene_count*self.gene_count):
            if random.uniform(0, 1) < 0.01:
                ...
                self.interaction_matrix[i] += np.random.normal(0, 0.35)
